...
fn show_expansion_generated_macro_rules_alpha() {
    ...
    if find_blocks(&blocks, "__trace_generated_alpha!").is_empty() {
        return;
    }
...
fn show_expansion_generated_macro_rules_beta() {
    let blocks = run_show_expansion();
    if find_blocks(&blocks, "__trace_generated_beta!").is_empty() {
        return;
    }
...
fn show_expansion_generated_macro_rules_are_distinct() {
    let blocks = run_show_expansion();
    if find_blocks(&blocks, "__trace_generated_alpha!").is_empty()
        || find_blocks(&blocks, "__trace_generated_beta!").is_empty()
    {
        return;
    }
...
fn show_expansion_generated_macro_rules_gamma_delta() {
    let blocks = run_show_expansion();
    if find_blocks(&blocks, "__trace_generated_gamma!").is_empty()
        || find_blocks(&blocks, "__trace_generated_delta!").is_empty()
    {
        return;
    }
...
fn show_expansion_decl_trace_alpha_and_beta() {
    let blocks = run_show_expansion();
    if find_blocks(&blocks, "decl_trace!").is_empty() {
        return;
    }
...
fn show_expansion_all_generated_macros_present() {
    ...
    if find_blocks(&blocks, "__trace_generated_alpha!").is_empty()
        || find_blocks(&blocks, "__trace_generated_beta!").is_empty()
        || find_blocks(&blocks, "__trace_generated_gamma!").is_empty()
        || find_blocks(&blocks, "__trace_generated_delta!").is_empty()
    {
        return;
    }
