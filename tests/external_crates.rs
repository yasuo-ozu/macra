...
/// Per-repo locks to prevent parallel `cargo check` invocations from racing
/// over the same target directory.  On Windows, parallel hook injection into
/// concurrent rustc processes sharing a build dir can cause intermittent
/// failures where hook output is lost.
mod repo_locks {
    use std::sync::Mutex;
    ...
    repo_lock!(DECYCLE);
    repo_lock!(DISCRIMINANT);
    repo_lock!(FLAT_ENUM);
    repo_lock!(NEWER_TYPE);
    repo_lock!(PARAMETRIZED);
    repo_lock!(SUMTYPE);
}

fn repo_lock(repo: &str) -> &'static Mutex<()> {
    match repo {
        "addr_of_enum" => &repo_locks::ADDR_OF_ENUM,
        "coinduction" => &repo_locks::COINDUCTION,
        "decycle" => &repo_locks::DECYCLE,
        "discriminant" => &repo_locks::DISCRIMINANT,
        "flat_enum/testing" => &repo_locks::FLAT_ENUM,
        "newer-type" => &repo_locks::NEWER_TYPE,
        "parametrized" => &repo_locks::PARAMETRIZED,
        "sumtype" => &repo_locks::SUMTYPE,
        other => panic!("unknown repo: {other}"),
    }
}
...
/// Crates whose transitive deps use `use<...>` precise capturing (stabilized
/// in 1.87) cannot compile on older toolchains.
fn min_rustc_for_crate(repo: &str) -> u32 {
    match repo {
        "coinduction" | "decycle" | "parametrized" | "sumtype" => 87,
        _ => 86,
    }
}
...
fn run_trace_for_repo(repo: &str, test: Option<&str>) -> Vec<MacroExpansion> {
    ...
    if !check_result.success {
        ...
        if rustc_minor_version() < min {
            eprintln!(
                "skipping external_crate `{repo}` test `{test:?}`: \
                 requires rustc >= 1.{min}, got 1.{}",
                rustc_minor_version()
            );
            return Vec::new();
        }
