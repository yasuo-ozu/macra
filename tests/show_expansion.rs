...
/// Normalize whitespace around `{` and `}` delimiters so that comparisons
/// are resilient to differences in `TokenStream::to_string()` formatting
/// across Rust versions.  Rust 1.86-1.87 formats groups as `{ … }` while
/// 1.88+ uses `{…}`.
fn normalize_delimiters(s: &str) -> String {
    s.replace("{ ", "{").replace(" }", "}")
}
...
fn assert_exact(expansions: &[MacroExpansion], caller: &str, expected_input: &str, expected_to: &str) {
    ...
    let matching: Vec<_> = expansions
        .iter()
        .filter(|e| {
            normalize_delimiters(&expansion_caller(e)) == norm_caller
                && normalize_delimiters(e.input.trim()) == norm_input.trim()
        })
        .collect();
    assert_eq!(
        matching.len(),
    ...
    assert_eq!(
        normalize_delimiters(matching[0].to.trim()),
        normalize_delimiters(expected_to.trim()),
